...
import deleteWorkFile from "../api/deleteWorkFile";
import { useAdminContext } from "../../../Context/AdminContext";

const FileCard = ({ id, file, file_name, download_url }) => {
  const [viewableType, setViewableType] = useState(false);
  const fileExtension = file_name.split(".")[file_name.split(".").length - 1];
  const { setWorkFiles } = useAdminContext();
  ...
  const handleDelete = (e) => {
    ...
    deleteWorkFile(id).then((data) => {
      console.log(data);
    });
    // Delete file from the state
    setWorkFiles((currentFiles) => {
      const newList = currentFiles.filter((file) => file.id !== id);
      return newList;
    });
  };
  return (
      ...
      <div className="flex items-center justify-between text-[15px] gap-4 p-3 bg-white hover:bg-neutral-100">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`${
              viewableType ? "" : "text-gray-400"
            } hover:bg-gray-200 rounded-full p-2 cursor-pointer transition-colors duration-300`}
            disabled={viewableType == false}
          >
            <a
              className={`${!viewableType ? "hidden" : ""}`}
              href={file}
              target="blank"
            >
              <Eye size={17} />
