...
const SubmissionCard = ({
  ...
  return (
    <>
      <div
        className=" relative pt-2 pb-16 px-[0.7rem] bg-white hover:bg-[#f1f1f1] dark:hover:bg-darkMode-cardHover
         transition-colors duration-300 dark:bg-darkMode-cardBg  "
        onClick={() => navigate(`/my-submissions/${id}`)}
      >
        <p className="mb-2 text-[14px] lg:text-[16px]">
          Submission for work
          <span
            onClick={(e) => {
              e.stopPropagation();
        ...
        >
          {claimed_by.registration_number ? (
            ...
            <div className="font-medium text-right text-[12px] lg:text-sm text-neutral-600 dark:text-white w-full ml-10">
              Not reviewed yet.
            </div>
          )}
        </div>
